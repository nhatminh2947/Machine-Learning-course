...
def draw(train_x, train_y, test_x, test_y, test_y_minus, test_y_plus, params, mode):
    # Plot result
    fig = plt.figure()
    plt.title("Gaussian Process with scalelength : {0:.2f} sigma : {1:.2f} alpha : {2:.2f}".format(params[0], params[1],
                                                                                                   params[2]))
...
if __name__ == "__main__":
    ...
    fmin = minimize(fun=NLL, x0=params, args=(train_x, train_y), bounds=((1e-3, None), (1e-3, None), (1e-3, None)),
