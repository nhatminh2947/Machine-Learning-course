...
def open_file():
    ...
    data = np.fromfile("./data/train-images-idx3-ubyte", dtype="ubyte")
    X = data[4 * data_type.itemsize:].astype("float64").reshape(60000, 28 * 28).transpose()
    X = np.divide(X, 128).astype("int")

    labels = np.fromfile("./data/train-labels-idx1-ubyte", dtype="ubyte").astype("int")
...
def print_confusion_matrix(X, labels, MU, PI, relation):
    ...
    for i in range(0, 10):
        ...
        print("Specificity (Successfully predict not number {})\t: {}".format(i, confusion_matrix[i][1][1] / (
