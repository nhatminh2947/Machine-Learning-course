from src.HW4.univariate_rng import UnivariateRNG
import numpy as np
...
class LogisticRegression(object):
    ...
    def __call__(self, x, y):
        return 1 / (1 + np.exp(
            - np.dot(np.array([x ** i for i in range(self.basis)] + [y ** i for i in range(self.basis)]), self.w)))
        ...
        for j in range(self.params):
            for k in range(self.params):
                for i in range(self.n):
                    ...
                    hess[j, k] += self.phi[i, j] * self.phi[i, k] * (self.logistic(np.dot(self.phi[i, :], self.w)) * (
                                1 - self.logistic(np.dot(self.phi[i, :], self.w))))
        return hess
...
def confusion_matrix(predicted, expected, n_classes=None):
    if n_classes is None:
        n_classes = int(
            np.amax([np.amax(expected), np.amax(predicted)]) - np.amin([np.amin(expected), np.amin(predicted)])) + 1
...
if __name__ == '__main__':
    ...
    for i in range(1000000):
        ...
        if i % 50 == 1:
            ...
            p = np.array(
                [int(l(l.x1[i], l.y1[i]) > 0.5) for i in range(n)] + [int(l(l.x2[i], l.y2[i]) > 0.5) for i in range(n)])
            e = l.d
